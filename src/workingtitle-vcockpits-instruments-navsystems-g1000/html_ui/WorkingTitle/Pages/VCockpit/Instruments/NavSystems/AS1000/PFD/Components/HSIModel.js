class HSIIndicatorModel {
    ...
    initDme(update$) {
        const sourceId$ = WT_RX.observeSimVar(update$, "L:Glasscockpit_DmeSource", "Number");

        const display$ = WT_RX.observeSimVar(update$, "L:PFD_DME_Displayed", "number").pipe(
            rxjs.operators.map(v => v != 0),
            WT_RX.shareReplay()
        );

        const source$ = sourceId$.pipe(
        ...
        const nav1HasDme$ = WT_RX.observeSimVar(update$, "NAV HAS DME:1", "Bool");
        const nav2HasDme$ = WT_RX.observeSimVar(update$, "NAV HAS DME:2", "Bool");
        const nav1Ident$ = WT_RX.observeSimVar(update$, "NAV ACTIVE FREQUENCY:1", "MHz").pipe(
            rxjs.operators.map(mhz => mhz.toFixed(2))
        );
        const nav2Ident$ = WT_RX.observeSimVar(update$, "NAV ACTIVE FREQUENCY:2", "MHz").pipe(
            rxjs.operators.map(mhz => mhz.toFixed(2))
        );

        const ident$ = sourceId$.pipe(
            rxjs.operators.switchMap(sourceId => {
                switch (sourceId) {
                    case 0:
                    case 1:
                    ...
                    case 2:
                        return nav2HasDme$.pipe(WT_RX.toggleSwitchMap(nav2Ident$, rxjs.of("")));
                }
                throw new Error(`Invalid source ID: ${sourceId}`);
            })
        );

        const nav1Distance$ = WT_RX.observeSimVar(update$, "NAV DME:1", "nautical miles");
        const nav2Distance$ = WT_RX.observeSimVar(update$, "NAV DME:2", "nautical miles");

        const distance$ = sourceId$.pipe(
            rxjs.operators.switchMap(sourceId => {
                switch (sourceId) {
                    case 0:
                        return rxjs.empty();
                    case 1:
                        return nav1HasDme$.pipe(WT_RX.toggleSwitchMap(nav1Distance$, rxjs.of(null)));
                    case 2:
                        return nav2HasDme$.pipe(WT_RX.toggleSwitchMap(nav2Distance$, rxjs.of(null)));
                }
                throw new Error(`Invalid source ID: ${sourceId}`);
            })
        );
        ...
        return {
            source: display$.pipe(WT_RX.toggleSwitchMap(source$, rxjs.empty())),
            ident: display$.pipe(WT_RX.toggleSwitchMap(ident$, rxjs.empty())),
            distance: display$.pipe(WT_RX.toggleSwitchMap(distance$, rxjs.empty())),
            display: display$,
        }
    }
    update(dt) {
    ...
    toggleDme() {
        const current = SimVar.GetSimVarValue("L:PFD_DME_Displayed", "number");
        const display = (current == 1) ? 0 : 1;
