class WT_Departure_Page_View extends WT_HTML_View {
    constructor() {
        super();
    ...
    setAirport(airport) {
        ...
        if (airport) {
            this.airportSubscription = airport.subscribe(async airport => {
                this.procedures.value = airport ? await airport.getDepartures() : [];
            });
        }
    }
    /**
     * @param {WT_Departure_Procedure[]} procedures 
     */
    updateProcedures(procedures) {
        try {
            console.log(`adding ${procedures.length} departure procedures`);
    ...
    setProcedure(procedureIndex) {
        this.selectedProcedure.value = new WT_Selected_Departure_Procedure(this.procedures.value[procedureIndex]);
    }
