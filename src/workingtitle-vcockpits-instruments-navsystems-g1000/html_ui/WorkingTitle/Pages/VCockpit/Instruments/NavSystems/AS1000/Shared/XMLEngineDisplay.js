...
class XMLCircularGauge extends XMLGauge {
    ...
    updateValue(_value) {
        if (Math.abs(_value - this.lastValue) > (1 / Math.pow(10, this.textPrecision))) {
            this.cursor.setAttribute("transform", "rotate(" + this.valueToAngle(Math.max(Math.min(_value, this.maxValue), this.minValue)) + " 50 40)");
            this.valueText.textContent = this.textIncrement != 1 ? fastToFixed(Math.round(_value / this.textIncrement) * this.textIncrement, this.textPrecision) : fastToFixed(_value, this.textPrecision);
...
class XMLHorizontalGauge extends XMLGauge {
        ...
        const handleValue = (value$, cursor, label, textElement, cautionBg, alertBg) => {
            ...
            if (textElement) {
                value$.pipe(
                    ...
                    rxjs.operators.map(() => textElement.getBBox()),
                    rxjs.operators.distinctUntilChanged((a, b) => a.x == b.x && a.y == b.y && a.width == b.width && a.height == b.height),
                ).subscribe(valueBbox => {
                    cautionBg.setAttribute("x", valueBbox.x - 1);
                    cautionBg.setAttribute("y", valueBbox.y - 1);
                    cautionBg.setAttribute("width", valueBbox.width + 2);
                    cautionBg.setAttribute("height", valueBbox.height + 2);
                    alertBg.setAttribute("x", valueBbox.x - 1);
                    alertBg.setAttribute("y", valueBbox.y - 1);
                    alertBg.setAttribute("width", valueBbox.width + 2);
                    alertBg.setAttribute("height", valueBbox.height + 2);
                });
                ...
                value$.pipe(
                    ...
                    rxjs.operators.distinctUntilChanged(),
                ).subscribe(color => textElement.setAttribute("fill", color == null ? "white" : color));
            }
        }
    ...
    drawBase() {
        ...
        this.cursor.setAttribute("stroke-width", "0.5");
        this.cursor.setAttribute("stroke", "#333");
        this.rootSvg.appendChild(this.cursor);
        this.beginText = document.createElementNS(Avionics.SVG.NS, "text");
        ...
        this.beginText.setAttribute("fill", "white");
        this.beginText.setAttribute("font-size", "8");
        this.beginText.setAttribute("font-family", "Roboto-Regular");
        this.beginText.setAttribute("text-anchor", "middle");
        this.rootSvg.appendChild(this.beginText);
        this.endText = document.createElementNS(Avionics.SVG.NS, "text");
        ...
        this.endText.setAttribute("fill", "white");
        this.endText.setAttribute("font-size", "8");
        this.endText.setAttribute("font-family", "Roboto-Regular");
        this.endText.setAttribute("text-anchor", "middle");
        this.rootSvg.appendChild(this.endText);
        this.titleText_cautionbg = document.createElementNS(Avionics.SVG.NS, "rect");
        this.titleText_cautionbg.setAttribute("fill-opacity", "0");
        ...
        this.titleText.setAttribute("font-family", "Roboto-Regular");
        this.titleText.setAttribute("CautionBlink", "Text");
        this.titleText.setAttribute("AlertBlink", "Text");
        this.rootSvg.appendChild(this.titleText);
        this.valueText_cautionbg = document.createElementNS(Avionics.SVG.NS, "rect");
        ...
        switch (this.valuePos) {
            case 1:
                ...
                this.valueText.setAttribute("fill", "white");
                this.valueText.setAttribute("font-size", "9");
                this.valueText.setAttribute("font-family", "Roboto-Regular");
                this.valueText.setAttribute("text-anchor", "end");
                this.valueText.setAttribute("CautionBlink", "Text");
                this.valueText.setAttribute("AlertBlink", "Text");
                this.rootSvg.appendChild(this.valueText);
...
class XMLHorizontalDoubleGauge extends XMLGauge {
    ...
    setupObservers() {
        ...
        handleValue(this.value1$, this.cursor, this.cursorLabel, this.valueText, this.valueText_cautionbg, this.valueText_alertbg);
        handleValue(this.value2$, this.cursor2, this.cursor2Label, this.valueText2, this.valueText2_cautionbg, this.valueText2_alertbg);
    }
    setStyle(_styleElem) {
        if (_styleElem) {
            let textIncrementElem = _styleElem.getElementsByTagName("TextIncrement");
    ...
    drawBase() {
        ...
        this.cursor2.setAttribute("stroke-width", "0.5");
        this.cursor2.setAttribute("stroke", "#333");
        ...
        switch (this.valuePos) {
            case 2:
                ...
                this.valueText2.setAttribute("font-family", "Roboto-Regular");
    ...
    setCursorLabel(_label1, _label2) {
        if (!this.cursorLabel) {
            ...
            this.cursorLabel.setAttribute("font-family", "Roboto-Regular");
            this.cursorLabel.setAttribute("text-anchor", "middle");
            this.rootSvg.appendChild(this.cursorLabel);
        }
        this.cursorLabel.textContent = _label1;
        if (_label2) {
            if (!this.cursor2Label) {
                ...
                this.cursor2Label.setAttribute("font-family", "Roboto-Regular");
