...
class FacilityLoader {
    constructor(_instrument) {
        ...
        this._maxSimultaneousCoherentCalls = 100;
    ...
    addFacility(_data) {
        ...
        if (!this._loadedIcaos[_data.icaoTrimed]) {
            this.loadedFacilities.push(_data);
            this._loadedIcaos[_data.icaoTrimed] = true;

            if (this.loadedFacilities.length > 1000) {
                const removed = this.loadedFacilities.splice(0, 1);
                delete this._loadedIcaos[removed[0].icaoTrimed];
            }
        } 
    }
    ...
    getAirportDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            let airport = this.loadedFacilities.find(f => { return f.icaoTrimed === icao;});
            if (airport) {
    ...
    getNdbDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            let ndb = this.loadedFacilities.find(f => { return f.icaoTrimed === icao;});
            if (ndb) {
                return callback(ndb);
            }
            if (icao[0] !== "N") {
    ...
    getVorDataCB(icao, callback) {
        if (this._isCompletelyRegistered && this.loadingFacilities.length < this._maxSimultaneousCoherentCalls) {
            icao = icao.trim();
            let vor = this.loadedFacilities.find(f => { return f.icaoTrimed === icao;});
            if (vor) {
                return callback(vor);
            }
            if (icao[0] !== "V") {
