class FMCMainDisplay extends BaseAirliners {
    ...
    static getQueryStringValue (key) {  
        let url = document.getElementsByTagName("cj4-fmc")[0].getAttribute("url");
        if(!url) return -3;
    ...
    onEvent(_event) {
        this._index = FMCMainDisplay.getQueryStringValue("idx");
        if(_event[0] !== this._index) return;
        ...
        if (_event.indexOf("1_BTN_") !== -1 || _event.indexOf("2_BTN_") !== -1 || _event.indexOf("BTN_") !== -1) {
            let input = _event.replace("1_BTN_", "");
            input = input.replace("2_BTN_", "");
            input = input.replace("BTN_", "");
