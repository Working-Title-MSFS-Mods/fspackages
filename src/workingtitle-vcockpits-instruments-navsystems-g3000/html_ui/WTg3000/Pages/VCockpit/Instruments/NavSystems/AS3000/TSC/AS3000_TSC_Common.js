...
class AS3000_TSC extends NavSystemTouch {
    ...
    connectedCallback() {
        ...
        this.dynamicSelectionListWindow1 = new NavSystemElementContainer("Dynamic Selection List Window 1", "DynamicSelectionListWindow1", new AS3000_TSC_DynamicSelectionListWindow());
        this.dynamicSelectionListWindow1.setGPS(this);
        this.dynamicSelectionListWindow2 = new NavSystemElementContainer("Dynamic Selection List Window 2", "DynamicSelectionListWindow2", new AS3000_TSC_DynamicSelectionListWindow());
        this.dynamicSelectionListWindow2.setGPS(this);
...
class AS3000_TSC_AvionicsMFDFieldsTab {
    ...
    onButtonClick(index) {
        ...
        this.parentElement.gps.dynamicSelectionListWindow1.element.setContext(context);
...
class AS3000_TSC_PFDSettings extends NavSystemElement {
    ...
    openBaroUnitSelectWindow() {
        ...
        this.gps.dynamicSelectionListWindow1.element.setContext(context);
        this.gps.switchToPopUpPage(this.gps.dynamicSelectionListWindow1);
    }
...
class AS3000_TSC_MapSettings extends NavSystemElement {
    constructor(
        ...
        this.tabs = [
            this._sensorTab = new WT_TSCMapSettingsTab(this, "MapSensorTab"),
        ...
        this._sensorTab.attachRow(new WT_TSCMapSettingsRangeTabRow("NEXRAD Data", WT_G3x5NavMap.NEXRAD_SHOW_KEY, WT_G3x5NavMap.NEXRAD_RANGE_KEY, WT_G3x5NavMap.NEXRAD_RANGE_MAX, "Map NEXRAD Range"));

        this._aviationTab.attachRow(new WT_TSCMapSettingsRangeTabRow("Airways", WT_G3x5NavMap.AIRWAY_SHOW_KEY, WT_G3x5NavMap.AIRWAY_RANGE_KEY, WT_G3x5NavMap.AIRWAY_RANGE_MAX, "Map Airway Range"));
        ...
        this._aviationTab.attachRow(new WT_TSCMapSettingsRangeTabRow("VOR", WT_G3x5NavMap.VOR_SHOW_KEY, WT_G3x5NavMap.VOR_RANGE_KEY, WT_G3x5NavMap.VOR_RANGE_MAX, "Map VOR Range"));
        this._aviationTab.attachRow(new WT_TSCMapSettingsRangeTabRow("INT", WT_G3x5NavMap.INT_SHOW_KEY, WT_G3x5NavMap.INT_RANGE_KEY, WT_G3x5NavMap.INT_RANGE_MAX, "Map INT Range"));
        this._aviationTab.attachRow(new WT_TSCMapSettingsRangeTabRow("NDB", WT_G3x5NavMap.NDB_SHOW_KEY, WT_G3x5NavMap.NDB_RANGE_KEY, WT_G3x5NavMap.NDB_RANGE_MAX, "Map NDB Range"));
        ...
        this._landTab.attachRow(new WT_TSCMapSettingsRangeTabRow("States/Provinces", WT_G3x5NavMap.BORDERS_SHOW_KEY, WT_G3x5NavMap.BORDERS_RANGE_KEY, WT_G3x5NavMap.BORDERS_RANGE_MAX, "Map State/Province Range"));

        this._otherTab.attachRow(new WT_TSCMapSettingsRangeTabRow("North Up<br>Above", WT_MapAutoNorthUpSettingGroup.ACTIVE_KEY, WT_MapAutoNorthUpSettingGroup.RANGE_KEY, WT_G3x5NavMap.MAP_RANGE_LEVELS[WT_G3x5NavMap.MAP_RANGE_LEVELS.length - 1], "Map North Up Above"));
        ...
        this._otherTab.attachRow(new WT_TSCMapSettingsFuelRingTabRow("Fuel Rng (Rsv)", WT_MapFuelRingSettingGroup.SHOW_KEY, WT_MapFuelRingSettingGroup.RESERVE_KEY));
        this._otherTab.attachRow(new WT_TSCMapSettingsToggleTabRow("Range to<br>Altitude", WT_MapAltitudeInterceptSetting.SHOW_KEY_DEFAULT));
    ...
    _createWindowContext(titleText, values, callback, key) {
        ...
        return {
            ...
            elementConstructor: elementHandler,
            elementUpdater: elementHandler,
            currentIndexGetter: new WT_MapSettingIndexGetter(this.controllerID, key),
            homePageParent: this.homePageParent,
            homePageName: this.homePageName
        };
    }

    initOrientationSetting() {
        this._orientationWindowContext = this._createWindowContext("Map Orientation", AS3000_TSC_MapSettings.ORIENTATION_TEXTS, this.setOrientation.bind(this), WT_G3x5NavMap.ORIENTATION_KEY);
    ...
    initSyncSetting() {
        this._syncWindowContext = this._createWindowContext("Map Sync", AS3000_TSC_MapSettings.SYNC_TEXTS, this.setSync.bind(this), WT_MapController.SYNC_MODE_KEY);
    ...
    updateOrientationValue() {
        let currentOrientation = WT_MapController.getSettingValue(this.controllerID, WT_G3x5NavMap.ORIENTATION_KEY);
        let newValue = "";
    ...
    updateSyncValue() {
        let currentSync = WT_MapController.getSyncMode(this.controllerID);
        let newValue = "";
        switch (currentSync) {
    ...
    updateDetailValue() {
        let currentDetail = WT_MapController.getSettingValue(this.controllerID, WT_MapDCLTRSetting.KEY_DEFAULT);
    ...
    openOrientationSelection() {
        this.gps.dynamicSelectionListWindow1.element.setContext(this._orientationWindowContext);
        this.gps.switchToPopUpPage(this.gps.dynamicSelectionListWindow1);
    }

    openSyncSelection() {
        this.gps.dynamicSelectionListWindow1.element.setContext(this._syncWindowContext);
        this.gps.switchToPopUpPage(this.gps.dynamicSelectionListWindow1);
    }

    openDetailSelection() {
        this.gps.mapDetailSelect.element.setContext(this.controllerID, this.homePageParent, this.homePageName);
    ...
    setOrientation(value) {
        WT_MapController.setSettingValue(this.controllerID, WT_G3x5NavMap.ORIENTATION_KEY, value, true);
        this.updateOrientationValue();
    }

    setSync(mode) {
        let oldMode = WT_MapController.getSyncMode(this.controllerID);
        if (mode !== oldMode) {
            switch (mode) {
                case WT_MapController.SyncMode.ALL:
                    for (let id of this._allControllerIDs) {
                        WT_MapController.setSyncMode(id, mode, this.controllerID);
                    }
                    break;
                case WT_MapController.SyncMode.OFF:
                    for (let id of this._allControllerIDs) {
    ...
    static getRangeValuesDisplayToMax(max) {
        ...
        for (let i = 0; i < WT_G3x5NavMap.MAP_RANGE_LEVELS.length && WT_G3x5NavMap.MAP_RANGE_LEVELS[i].compare(max) <= 0; i++) {
            values.push(AS3000_TSC_MapSettings.getRangeValueText(WT_G3x5NavMap.MAP_RANGE_LEVELS[i]));
        }
        return values;
    }
}
AS3000_TSC_MapSettings.ORIENTATION_TEXTS = [
    "Heading Up",
    "Track Up",
    "North Up"
];
AS3000_TSC_MapSettings.SYNC_TEXTS = [
    "Off",
    "All"
];
...
class AS3000_TSC_MapDetailSelect extends NavSystemElement {
    ...
    setContext(controllerID, _homePageParent, _homePageName) {
...
class WT_TSCMapSettingsTabElement extends HTMLElement {
    constructor() {
        super();
...
class WT_TSCMapSettingsTab {
    constructor(settingsPage, elementName) {
        this._settingsPage = settingsPage;
        this._elementName = elementName;

        this._htmlElement = new WT_TSCMapSettingsTabElement();

        /**
         * @type {WT_TSCMapSettingsTabRow[]}
         */
        this._rows = [];
     ...
     * @type {WT_TSCMapSettingsTabElement}
     */
    get htmlElement() {
    ...
    _createContext() {
        return {
            ...
            controllerID: this._settingsPage.controllerID,
            homePageGroup: this._settingsPage.homePageParent,
    ...
    /**
     *
     * @param {WT_TSCMapSettingsTabRow} row
     */
    _initRow(row) {
...
class WT_TSCMapSettingsTabRowElement extends HTMLElement {
    constructor() {
        super();
    ...
    setLeft(element) {
        if (this.left && this.left.parentNode === this) {
            this.left.classList.remove(WT_TSCMapSettingsTabRowElement.LEFT_CLASS);
            this.removeChild(this.left);
        }
        this.appendChild(element);
        element.setAttribute("slot", "left");
        element.classList.add(WT_TSCMapSettingsTabRowElement.LEFT_CLASS);
        this._left = element;
    }

    setRight(element) {
        if (this.right && this.right.parentNode === this) {
            this.right.classList.remove(WT_TSCMapSettingsTabRowElement.RIGHT_CLASS);
            this._wrapper.removeChild(this.right);
        }
        this.appendChild(element);
        element.setAttribute("slot", "right");
        element.classList.add(WT_TSCMapSettingsTabRowElement.RIGHT_CLASS);
        this._right = element;
    }
}
...
customElements.define("tsc-mapsettings-tabrow", WT_TSCMapSettingsTabRowElement);

class WT_TSCMapSettingsTabRow  {
    constructor() {
        this._htmlElement = new WT_TSCMapSettingsTabRowElement();
     ...
     * @type {WT_TSCMapSettingsTabRowElement}
     */
    get htmlElement() {
        return this._htmlElement;
    }
...
class WT_TSCMapSettingsToggleTabRow extends WT_TSCMapSettingsTabRow {
    /**
    ...
    _onToggleButtonPressed() {
        WT_MapController.setSettingValue(this.context.controllerID, this._toggleSettingKey, !WT_MapController.getSettingValue(this.context.controllerID, this._toggleSettingKey, false), true);
    }

    _updateToggleButton() {
        this._toggleButton.toggle = WT_MapController.getSettingValue(this.context.controllerID, this._toggleSettingKey, false) ? "on" : "off";
    }
...
class WT_TSCMapSettingsRangeTabRow extends WT_TSCMapSettingsToggleTabRow {
    /**
    ...
    _initWindowContext() {
        let elementHandler = new WT_TouchDynamicSelectionStandardElementHandler(WT_TSCMapSettingsRangeTabRow.getRangeValuesDisplayToMax(this._rangeMax));
        this._rangeWindowContext = {
            title: this._rangeWindowTitleText,
            subclass: "standardDynamicSelectionListWindow",
            closeOnSelect: true,
            callback: this._setRangeSetting.bind(this, this.context.controllerID),
            elementConstructor: elementHandler,
            elementUpdater: elementHandler,
            currentIndexGetter: new WT_MapSettingIndexGetter(this.context.controllerID, this._rangeKey),
    ...
    _updateRangeButton() {
        this._rangeButton.labelText = WT_TSCMapSettingsRangeTabRow.getRangeValueText(WT_G3x5NavMap.MAP_RANGE_LEVELS[WT_MapController.getSettingValue(this.context.controllerID, this._rangeKey)]);
    }
    ...
    static getRangeValuesDisplayToMax(max) {
        let values = [];
        for (let i = 0; i < WT_G3x5NavMap.MAP_RANGE_LEVELS.length && WT_G3x5NavMap.MAP_RANGE_LEVELS[i].compare(max) <= 0; i++) {
            values.push(WT_TSCMapSettingsRangeTabRow.getRangeValueText(WT_G3x5NavMap.MAP_RANGE_LEVELS[i]));
        }
        return values;
    }
}

class WT_TSCMapSettingsMultiRangeTabRow extends WT_TSCMapSettingsToggleTabRow {
    /**
     * @param {String} showButtonLabel
     * @param {String} showKey
    ...
    _initTypeWindowContext() {
        let elementHandler = new WT_TSCDynamicSelectionRangeTypeElementHandler(this.context.controllerID, this._rangeTypeNames, this._rangeKeys);
    ...
    _initRangeWindowContexts() {
        ...
        for (let i = 0; i < this._rangeKeys.length; i++) {
            let elementHandler = new WT_TouchDynamicSelectionStandardElementHandler(WT_TSCMapSettingsRangeTabRow.getRangeValuesDisplayToMax(this._rangesMax[i]));
            this._rangeWindowContexts[i] = {
                ...
                elementConstructor: elementHandler,
                elementUpdater: elementHandler,
                currentIndexGetter: new WT_MapSettingIndexGetter(this.context.controllerID, this._rangeKeys[i]),
                homePageParent: this.context.homePageGroup,
                homePageName: this.context.homePageName
            };
        }
    ...
    _openRangeWindow(index) {
        this.context.instrument.dynamicSelectionListWindow2.element.setContext(this._rangeWindowContexts[index]);
        this.context.instrument.switchToPopUpPage(this.context.instrument.dynamicSelectionListWindow2);
    }

    _setRangeSetting(index, value) {
        WT_MapController.setSettingValue(this.context.controllerID, this._rangeKeys[index], value, true);
    }

    _onRangeTypeButtonPressed() {
        this.context.instrument.dynamicSelectionListWindow1.element.setContext(this._typeWindowContext);
        this.context.instrument.switchToPopUpPage(this.context.instrument.dynamicSelectionListWindow1);
    }
...
class WT_TSCDynamicSelectionRangeTypeElementHandler {
    constructor(controllerID, typeNames, rangeKeys) {
        this._controllerID = controllerID;
    ...
    update(index, elem) {
        Avionics.Utils.diffAndSet(elem.value, WT_TSCMapSettingsRangeTabRow.getRangeValueText(WT_G3x5NavMap.MAP_RANGE_LEVELS[WT_MapController.getSettingValue(this._controllerID, this._rangeKeys[index])]));
    }
}
...
class WT_TSCMapSettingsTrackVectorTabRow extends WT_TSCMapSettingsToggleTabRow {
    ...
    _updateLookaheadButton() {
        this._lookaheadButton.labelText = WT_TSCMapSettingsTrackVectorTabRow.getTrackVectorLookaheadText(this._lookaheadValues[WT_MapController.getSettingValue(this.context.controllerID, this._lookaheadSettingKey)]);
    }
...
class WT_TSCMapSettingsFuelRingTabRow extends WT_TSCMapSettingsToggleTabRow {
    ...
    _updateReserveButton() {
        this._reserveButton.labelText = WT_TSCMapSettingsFuelRingTabRow.getFuelRingReserveTimeText(WT_MapController.getSettingValue(this.context.controllerID, this._reserveTimeSettingKey));
    }
...
class WT_TSCMapSettingsTerrainTabRow extends WT_TSCMapSettingsTabRow {
    /**
     * @param {String} showButtonLabel
     * @param {String} showKey
     * @param {String} rangeKey
    ...
    _initWindowContext() {
        let elementHandler = new WT_TouchDynamicSelectionStandardElementHandler(WT_G3x5NavMap.TERRAIN_MODE_DISPLAY_TEXT);
        this._modeWindowContext = {
            ...
            elementConstructor: elementHandler,
            elementUpdater: elementHandler,
            currentIndexGetter: new WT_MapSettingIndexGetter(this.context.controllerID, this._modeSettingKey),
            homePageParent: this.context.homePageParent,
            homePageName: this.context.homePageName
        };
    }

    _setModeSetting(value) {
        WT_MapController.setSettingValue(this.context.controllerID, this._modeSettingKey, value, true);
    }

    _onModeButtonPressed() {
        this.context.instrument.dynamicSelectionListWindow1.element.setContext(this._modeWindowContext);
        this.context.instrument.switchToPopUpPage(this.context.instrument.dynamicSelectionListWindow1);
    }
    ...
    _updateModeButton() {
        this._modeButton.valueText = WT_G3x5NavMap.TERRAIN_MODE_DISPLAY_TEXT[WT_MapController.getSettingValue(this.context.controllerID, this._modeSettingKey)];
    }
...
class AS3000_TSC_DynamicSelectionListWindow extends WT_TouchDynamicSelectionListWindow {
    ...
    backHome() {
        this.gps.closePopUpElement();
        this.gps.SwitchToPageName(this.context.homePageParent, this.contex.homePageName);
    }
}

class WT_MapSettingIndexGetter {
    constructor(controllerID, key) {
        this._controllerID = controllerID;
        this._key = key;
    }

    getCurrentIndex() {
        return WT_MapController.getSettingValue(this._controllerID, this._key);
    }
}
