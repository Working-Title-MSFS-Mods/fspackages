...
class PFD_NavStatus extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (flightPlanActive) {
            ...
            diffAndSetHTML(this.currentLegTo, "");
            diffAndSetHTML(this.currentLegFrom, "");
            diffAndSetHTML(this.currentLegSymbol, "");
            diffAndSetHTML(this.currentLegDistance, "__._NM");
            diffAndSetHTML(this.currentLegBearing, "___°");
        }
    }
    onExit() {
    }
...
class PFD_ADF_DME extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (this.gps.currentInteractionState == 1 && this.gps.currentSelectableArray == this.defaultSelectables && this.gps.cursorIndex == 0) {
            diffAndSetHTML(this.indicationText, "ENT TO TRANSFER");
        }
        else {
            diffAndSetHTML(this.indicationText, "");
        }
    }
    onExit() {
        ...
        if (value != this.lastValue || mode != this.lastSource) {
            switch (mode) {
                case 0:
                    ...
                    break;
                case 1:
                    diffAndSetHTML(this.source, "BARO MIN");
                    diffAndSetHTML(this.value, value + '<span class="unit">FT</span>');
                    diffAndSetAttribute(this.altimeter, "minimum-altitude", value);
                    diffAndSetAttribute(this.window, "state", "Active");
                    break;
                case 2:
                    diffAndSetHTML(this.source, "COMP MIN");
                    diffAndSetHTML(this.value, value + '<span class="unit">FT</span>');
                    diffAndSetAttribute(this.altimeter, "minimum-altitude", value);
                    diffAndSetAttribute(this.window, "state", "Active");
                    break;
                case 3:
                    diffAndSetHTML(this.source, "RA MIN");
                    diffAndSetHTML(this.value, value + '<span class="unit">FT</span>');
                    diffAndSetAttribute(this.window, "state", "Active");
                    break;
            }
...
class PFD_RadarAltitude extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (radarAltitude > 0 && radarAltitude < 2500 && (Math.abs(xyz.bank) < Math.PI * 0.35)) {
            ...
            diffAndSetHTML(this.value, fastToFixed(radarAltitude, 0));
            diffAndSetAttribute(this.window, "state", "Active");
        }
        else {
    ...
    onUpdate(_deltaTime) {
        ...
        switch (state) {
            ...
            case 1:
                ...
                diffAndSetHTML(this.element, "O");
                break;
            case 2:
                diffAndSetAttribute(this.element, "state", "M");
                diffAndSetHTML(this.element, "M");
                break;
            case 3:
                diffAndSetAttribute(this.element, "state", "I");
                diffAndSetHTML(this.element, "I");
                break;
        }
    }
    onExit() {
...
class PFD_AutopilotDisplay extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        diffAndSetHTML(this.AP_YDStatus, SimVar.GetSimVarValue("AUTOPILOT YAW DAMPER", "Bool") ? "YD" : "");
        diffAndSetHTML(this.AP_Status, this.apStatusDisplay != 0 ? "AP" : "");
        ...
        if (SimVar.GetSimVarValue("AUTOPILOT PITCH HOLD", "Boolean")) {
            diffAndSetHTML(this.AP_VerticalActive, "PIT");
            diffAndSetHTML(this.AP_ModeReference, "");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT FLIGHT LEVEL CHANGE", "Boolean")) {
            diffAndSetHTML(this.AP_VerticalActive, "FLC");
            if (SimVar.GetSimVarValue("L:XMLVAR_AirSpeedIsInMach", "Boolean") || SimVar.GetSimVarValue("AUTOPILOT MANAGED SPEED IN MACH", "Boolean")) {
                let refMach = SimVar.GetSimVarValue("AUTOPILOT MACH HOLD VAR", "mach");
                diffAndSetHTML(this.AP_ModeReference, "M " + (refMach < 1 ? refMach.toFixed(3).slice(1) : refMach.toFixed(3)));
            }
            else {
                diffAndSetHTML(this.AP_ModeReference, fastToFixed(SimVar.GetSimVarValue("AUTOPILOT AIRSPEED HOLD VAR", "knots"), 0) + "KT");
            }
        }
            ...
            if (SimVar.GetSimVarValue("AUTOPILOT ALTITUDE ARM", "Boolean")) {
                diffAndSetHTML(this.AP_VerticalActive, "ALTS");
            }
            else {
                diffAndSetHTML(this.AP_VerticalActive, "ALT");
            }
            diffAndSetHTML(this.AP_ModeReference, fastToFixed(SimVar.GetSimVarValue("AUTOPILOT ALTITUDE LOCK VAR:2", "feet"), 0) + "FT");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT VERTICAL HOLD", "Boolean")) {
            diffAndSetHTML(this.AP_VerticalActive, "VS");
            diffAndSetHTML(this.AP_ModeReference, fastToFixed(SimVar.GetSimVarValue("AUTOPILOT VERTICAL HOLD VAR", "feet per minute"), 0) + "FPM");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT GLIDESLOPE ACTIVE", "Boolean")) {
            if (SimVar.GetSimVarValue("GPS DRIVES NAV1", "Boolean")) {
                diffAndSetHTML(this.AP_VerticalActive, "GP");
            }
            else {
                diffAndSetHTML(this.AP_VerticalActive, "GS");
            }
            diffAndSetHTML(this.AP_ModeReference, "");
        }
        else {
            diffAndSetHTML(this.AP_VerticalActive, "");
            diffAndSetHTML(this.AP_ModeReference, "");
        }
        if (SimVar.GetSimVarValue("AUTOPILOT ALTITUDE ARM", "Boolean")) {
            diffAndSetHTML(this.AP_Armed, "ALT");
            diffAndSetHTML(this.AP_ArmedReference, "");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT GLIDESLOPE ARM", "Boolean")) {
            if (SimVar.GetSimVarValue("GPS DRIVES NAV1", "Boolean")) {
                diffAndSetHTML(this.AP_Armed, "V ALT");
                diffAndSetHTML(this.AP_ArmedReference, "GP");
            }
            else {
                diffAndSetHTML(this.AP_Armed, "GS");
                diffAndSetHTML(this.AP_ArmedReference, "");
            }
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT VERTICAL HOLD", "Boolean")) {
            diffAndSetHTML(this.AP_Armed, "ALTS");
            diffAndSetHTML(this.AP_ArmedReference, "");
        }
        else {
            diffAndSetHTML(this.AP_Armed, "");
            diffAndSetHTML(this.AP_ArmedReference, "");
        }
        if (SimVar.GetSimVarValue("AUTOPILOT WING LEVELER", "Boolean")) {
            diffAndSetHTML(this.AP_LateralActive, "LVL");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT BANK HOLD", "Boolean")) {
            diffAndSetHTML(this.AP_LateralActive, "ROL");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT HEADING LOCK", "Boolean")) {
            diffAndSetHTML(this.AP_LateralActive, "HDG");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT NAV1 LOCK", "Boolean")) {
            if (SimVar.GetSimVarValue("GPS DRIVES NAV1", "Boolean")) {
                diffAndSetHTML(this.AP_LateralActive, "GPS");
            }
            else {
                if (SimVar.GetSimVarValue("NAV HAS LOCALIZER:" + Simplane.getAutoPilotSelectedNav(), "Boolean")) {
                    diffAndSetHTML(this.AP_LateralActive, "LOC");
                }
                else {
                    diffAndSetHTML(this.AP_LateralActive, "VOR");
                }
            }
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT BACKCOURSE HOLD", "Boolean")) {
            diffAndSetHTML(this.AP_LateralArmed, "BC");
        }
        else if (SimVar.GetSimVarValue("AUTOPILOT APPROACH HOLD", "Boolean")) {
            if (SimVar.GetSimVarValue("GPS DRIVES NAV1", "Boolean")) {
                diffAndSetHTML(this.AP_LateralArmed, "GPS");
            }
            else {
                if (SimVar.GetSimVarValue("NAV HAS LOCALIZER:" + Simplane.getAutoPilotSelectedNav(), "Boolean")) {
                    diffAndSetHTML(this.AP_LateralActive, "LOC");
            ...
            diffAndSetHTML(this.AP_LateralActive, "");
        }
        if (SimVar.GetSimVarValue("AUTOPILOT HEADING LOCK", "Bool") || SimVar.GetSimVarValue("AUTOPILOT WING LEVELER", "Bool")) {
            if (SimVar.GetSimVarValue("AUTOPILOT NAV1 LOCK", "Boolean")) {
                if (SimVar.GetSimVarValue("GPS DRIVES NAV1", "Boolean")) {
                    ...
                    if (SimVar.GetSimVarValue("NAV HAS LOCALIZER:" + Simplane.getAutoPilotSelectedNav(), "Boolean")) {
                        diffAndSetHTML(this.AP_LateralArmed, "LOC");
                    }
                    else {
                        diffAndSetHTML(this.AP_LateralArmed, "VOR");
                    }
                }
            }
            else if (SimVar.GetSimVarValue("AUTOPILOT BACKCOURSE HOLD", "Boolean")) {
                ...
                diffAndSetHTML(this.AP_LateralArmed, "");
            }
        }
        else {
            diffAndSetHTML(this.AP_LateralArmed, "");
...
class MFD_ActiveFlightPlan_Element extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        diffAndSetHTML(this.titleElement, (this.gps.currFlightPlanManager.getWaypointsCount() > 0 ? (this.gps.currFlightPlanManager.getWaypoint(0).infos.ident != "" ? this.gps.currFlightPlanManager.getWaypoint(0).infos.ident : this.gps.currFlightPlanManager.getWaypoint(0).ident) : "______") + "/" + (this.gps.currFlightPlanManager.getWaypointsCount() > 1 ? (this.gps.currFlightPlanManager.getWaypoint(this.gps.currFlightPlanManager.getWaypointsCount() - 1).infos.ident != "" ? this.gps.currFlightPlanManager.getWaypoint(this.gps.currFlightPlanManager.getWaypointsCount() - 1).infos.ident : this.gps.currFlightPlanManager.getWaypoint(this.gps.currFlightPlanManager.getWaypointsCount() - 1).ident) : "______"));
...
class MFD_DuplicateWaypoint extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        diffAndSetHTML(this.ident, this.icaoSearchField.duplicates[0].ident);
        let strings = [];
        ...
        if (info && info.icao != "") {
            diffAndSetHTML(this.nameElement, info.name);
            if (info.coordinates) {
                diffAndSetHTML(this.lat, this.gps.latitudeFormat(info.coordinates.lat));
                diffAndSetHTML(this.long, this.gps.longitudeFormat(info.coordinates.long));
                if (info.coordinates.lat && info.coordinates.long) {
                    ...
                    this.geoCalc.Compute(function () {
                        diffAndSetHTML(this.bearing, fastToFixed(this.geoCalc.bearing, 0) + "°");
                        diffAndSetHTML(this.distance, fastToFixed(this.geoCalc.distance, 0) + "NM");
                    }.bind(this));
                }
            }
        }
...
class MFD_NearestAirport_Element extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && infos.icao != "" && infos.getWaypointType() == "A" && infos.IsUpToDate()) {
            diffAndSetHTML(this.facility, infos.name);
            diffAndSetHTML(this.city, infos.city);
            if (infos.coordinates) {
                diffAndSetHTML(this.elevation, fastToFixed(infos.coordinates.alt, 0) + "FT");
            }
            if (infos.runways) {
                if (this.runwayIndex >= infos.runways.length) {
                    this.runwayIndex = 0;
                }
                diffAndSetHTML(this.runwayDesignation, infos.runways[this.runwayIndex].designation);
                diffAndSetHTML(this.runwaySurface, infos.runways[this.runwayIndex].getSurfaceString());
                diffAndSetHTML(this.runwayLength, fastToFixed(infos.runways[this.runwayIndex].length, 0) + "FT");
                diffAndSetHTML(this.runwayWidth, fastToFixed(infos.runways[this.runwayIndex].width, 0) + "FT");
            }
...
class MFD_NearestVOR_Element extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && infos.icao != "" && infos.getWaypointType() == "V" && infos.IsUpToDate()) {
            diffAndSetHTML(this.facility, infos.name);
            diffAndSetHTML(this.city, infos.city);
            diffAndSetHTML(this.class, infos.getClassName());
            let magVar = "";
            ...
            diffAndSetHTML(this.magvar, magVar);
            if (infos.coordinates) {
                diffAndSetHTML(this.latitude, this.gps.latitudeFormat(infos.coordinates.lat));
                diffAndSetHTML(this.longitude, this.gps.longitudeFormat(infos.coordinates.long));
            }
            diffAndSetHTML(this.frequency, infos.frequencyMHz ? infos.frequencyMHz.toFixed(2) : "___.__");
            this.gps.lastRelevantICAOType = this.currentWaypoint.type;
            this.gps.lastRelevantICAO = infos.icao;
        }
    }
...
class MFD_NearestNDB_Element extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && infos.icao != "" && infos.getWaypointType() == "N" && infos.IsUpToDate()) {
            diffAndSetHTML(this.facility, infos.name);
            diffAndSetHTML(this.city, infos.city);
            diffAndSetHTML(this.class, infos.getTypeString());
            if (infos.coordinates) {
                diffAndSetHTML(this.latitude, this.gps.latitudeFormat(infos.coordinates.lat));
                diffAndSetHTML(this.longitude, this.gps.longitudeFormat(infos.coordinates.long));
            }
            diffAndSetHTML(this.frequency, infos.frequencyMHz.toFixed(2));
            this.gps.lastRelevantICAOType = this.currentWaypoint.type;
            this.gps.lastRelevantICAO = infos.icao;
        }
    }
...
class MFD_NearestIntersection_Element extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && infos.icao != "" && infos.getWaypointType() == "W" && infos.IsUpToDate()) {
            ...
            diffAndSetHTML(this.vorIdent, infos.nearestVORIdent);
            ...
            diffAndSetHTML(this.vorFreq, infos.nearestVORFrequencyMHz.toFixed(2));
            diffAndSetHTML(this.vorBearing, fastToFixed(infos.nearestVORMagneticRadial, 0) + "°");
            diffAndSetHTML(this.vorDistance, fastToFixed(infos.nearestVORDistance / 1852, 1) + "NM");
            this.gps.lastRelevantICAOType = this.currentWaypoint.type;
            this.gps.lastRelevantICAO = infos.icao;
        }
    }
...
class MFD_Procedures extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (approach) {
            diffAndSetHTML(this.loadedApproach, approach.name);
        }
        else {
            diffAndSetHTML(this.loadedApproach, "____-");
        }
        let departure = this.gps.currFlightPlanManager.getDeparture();
        if (departure) {
            diffAndSetHTML(this.loadedDeparture, departure.name);
        }
        else {
            diffAndSetHTML(this.loadedDeparture, "____-");
        }
        let arrival = this.gps.currFlightPlanManager.getArrival();
        if (arrival) {
            diffAndSetHTML(this.loadedArrival, arrival.name);
        }
        else {
            diffAndSetHTML(this.loadedArrival, "____-");
        }
    }
    onExit() {
...
class MFD_ApproachSelection extends NavSystemElement {
        ...
        if (_event == "ENT_Push" || _event == "NavigationSmallInc" || _event == "NavigationSmallDec") {
            ...
            if (infos && infos.icao) {
                for (let i = 0; i < infos.approaches.length; i++) {
                    ...
                    diffAndSetHTML(this.approachLines[i], infos.approaches[i].name);
        ...
        if (_event == "ENT_Push" || _event == "NavigationSmallInc" || _event == "NavigationSmallDec") {
            ...
            if (approach && approach.transitions.length > 0) {
                for (let i = 0; i < approach.transitions.length; i++) {
                    ...
                    diffAndSetHTML(this.transitionLines[i], approach.transitions[i].name);
    ...
    onUpdate(_deltaTime) {
        ...
        let infos = this.icaoSearchField.getUpdatedInfos();
        if (infos && infos.icao) {
            diffAndSetHTML(this.elem_airportCity, infos.city);
            switch (infos.privateType) {
                case 0:
                    diffAndSetHTML(this.elem_airportType, "Unknown");
                    break;
                case 1:
                    diffAndSetHTML(this.elem_airportType, "Public");
                    break;
                case 2:
                    diffAndSetHTML(this.elem_airportType, "Military");
                    break;
                case 3:
                    diffAndSetHTML(this.elem_airportType, "Private");
                    break;
            }
            var logo = infos.imageFileName();
            if (logo != "") {
            ...
            let approach = this.getSelectedApproach(infos);
            if (approach) {
                diffAndSetHTML(this.elem_approach, approach.name);
                if (approach.transitions && this.selectedTransition >= 0 && approach.transitions.length > this.selectedTransition) {
                    diffAndSetHTML(this.elem_transition, approach.transitions[this.selectedTransition].name);
                }
                else {
                    diffAndSetHTML(this.elem_transition, "NONE");
                }
            }
            else {
                diffAndSetHTML(this.elem_approach, "NONE");
                diffAndSetHTML(this.elem_transition, "NONE");
            }
            if (this.elem_sequence && this.nbLines > 0) {
                let sequence = [];
            ...
            diffAndSetHTML(this.elem_airportCity, "____________");
            diffAndSetHTML(this.elem_airportType, "Unknown");
            diffAndSetAttribute(this.elem_airportLogo, "src", "");
            if (this.elem_sequence && this.nbLines > 0) {
                this.sequenceSliderGroup.setStringElements([]);
...
class MFD_ArrivalSelection extends NavSystemElement {
        ...
        if (_event == "ENT_Push" || _event == "NavigationSmallInc" || _event == "NavigationSmallDec") {
            ...
            if (arrival && arrival.enRouteTransitions.length > 0) {
                for (let i = 0; i < arrival.enRouteTransitions.length; i++) {
                    ...
                    diffAndSetHTML(this.transitionLines[i], arrival.enRouteTransitions[i].name);
                    diffAndSetAttribute(this.transitionLines[i], "state", "Active");
                    diffAndSetAttribute(this.transitionLines[i], "class", "Blinking");
                    this.transitionSelectables.push(new SelectableElement(this.gps, this.transitionLines[i], this.selectTransition.bind(this, i)));
                }
        ...
        if (_event == "ENT_Push" || _event == "NavigationSmallInc" || _event == "NavigationSmallDec") {
            ...
            if (arrival && arrival.runwayTransitions.length > 0) {
                for (let i = 0; i < arrival.runwayTransitions.length; i++) {
                    ...
                    diffAndSetHTML(this.runwayLines[i], arrival.runwayTransitions[i].name);
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && infos.icao) {
            ...
            let arrival = this.getSelectedArrival(infos);
            if (arrival) {
                diffAndSetHTML(this.elem_arrival, arrival.name);
                if (arrival.enRouteTransitions && this.selectedTransition >= 0 && arrival.enRouteTransitions.length > this.selectedTransition) {
                    diffAndSetHTML(this.elem_transition, arrival.enRouteTransitions[this.selectedTransition].name);
                }
                else {
                    diffAndSetHTML(this.elem_transition, "NONE");
                }
                if (arrival.runwayTransitions && this.selectedRunway >= 0 && arrival.runwayTransitions.length > this.selectedRunway) {
                    diffAndSetHTML(this.elem_runway, arrival.runwayTransitions[this.selectedRunway].name);
                }
                else {
                    diffAndSetHTML(this.elem_runway, "ALL");
                }
            }
            else {
                diffAndSetHTML(this.elem_arrival, "NONE");
                diffAndSetHTML(this.elem_transition, "NONE");
            }
            if (this.elem_sequence && this.nbLines > 0) {
                let sequence = [];
...
class MFD_DepartureSelection extends NavSystemElement {
        ...
        if (_event == "ENT_Push" || _event == "NavigationSmallInc" || _event == "NavigationSmallDec") {
            ...
            if (departure && departure.enRouteTransitions.length > 0) {
                for (let i = 0; i < departure.enRouteTransitions.length; i++) {
                    ...
                    diffAndSetHTML(this.transitionLines[i], departure.enRouteTransitions[i].name);
                    diffAndSetAttribute(this.transitionLines[i], "state", "Active");
                    diffAndSetAttribute(this.transitionLines[i], "class", "Blinking");
                    this.transitionSelectables.push(new SelectableElement(this.gps, this.transitionLines[i], this.selectTransition.bind(this, i)));
                }
        ...
        if (_event == "ENT_Push" || _event == "NavigationSmallInc" || _event == "NavigationSmallDec") {
            ...
            if (departure && departure.runwayTransitions.length > 0) {
                for (let i = 0; i < departure.runwayTransitions.length; i++) {
                    ...
                    diffAndSetHTML(this.runwayLines[i], departure.runwayTransitions[i].name);
                    diffAndSetAttribute(this.runwayLines[i], "state", "Active");
                    diffAndSetAttribute(this.runwayLines[i], "class", "Blinking");
                    this.runwaySelectables.push(new SelectableElement(this.gps, this.runwayLines[i], this.selectRunway.bind(this, i)));
                }
    ...
    onUpdate(_deltaTime) {
        ...
        if (infos && infos.icao) {
            ...
            let departure = this.getSelectedDeparture(infos);
            if (departure) {
                diffAndSetHTML(this.elem_departure, departure.name);
                if (this.selectedTransition >= 0 && departure.enRouteTransitions.length > this.selectedTransition) {
                    diffAndSetHTML(this.elem_transition, departure.enRouteTransitions[this.selectedTransition].name);
                }
                else {
                    diffAndSetHTML(this.elem_transition, "NONE");
                }
                if (this.selectedRunway >= 0 && departure.runwayTransitions.length > this.selectedRunway) {
                    diffAndSetHTML(this.elem_runway, departure.runwayTransitions[this.selectedRunway].name);
                }
                else {
                    diffAndSetHTML(this.elem_runway, "ALL");
                }
                ...
                diffAndSetHTML(this.elem_departure, "NONE");
                diffAndSetHTML(this.elem_transition, "NONE");
            }
            if (this.elem_sequence && this.nbLines > 0) {
                let sequence = [];
