class CJ4_FMC_DirectToPage {
    ...
    static ShowPage1(fmc, directWaypoint, page = 1, directWaypointOnFlightplan = false) {
        ...
        let modStr = "ACT[blue]";
        const onDirect = fmc.flightPlanManager.getIsDirectTo();
    ...
    static ShowPage2(fmc) {
        fmc.clearDisplay();
        const minRunwayValue = WTDataStore.get('WT_CJ4_MIN_NRST_RWY', 3000);
        const position = new LatLongAlt(SimVar.GetSimVarValue("GPS POSITION LAT", "degree latitude"), SimVar.GetSimVarValue("GPS POSITION LON", "degree longitude"));

        if (fmc._nearest && fmc._nearest.airports.length > 0) {
            const airportsFiltered = [...fmc._nearest.airports].filter(w => w.ident == "origin" || w.ident == "destination"
            ...
            if (fpOrigin) {
                const origin = {
                    ...
                    latitude: 0,
                    longitude: 0,
                    longestRunway: airportsSorted.find(a => {
                        return a.ident == "origin";
                    }).longestRunway,
                    longestRunwayLength: airportsSorted.find(a => {
                        return a.ident == "origin";
                    }).longestRunwayLength
                };
            ...
            for (let i = 0; i < 5; i++) {
                const airport = closestAirports[i];
                ...
                fmc.onLeftInput[i] = () => {
                    if (waypointsCell[i] != "") {
                        ...
                        const icao = closestAirports[i].icao;
                        const currentOrigin = fmc.flightPlanManager.getOrigin();
                        fmc.ensureCurrentFlightPlanIsTemporary(() => {
                            fmc.flightPlanManager.clearFlightPlan(() => {
                                fmc.flightPlanManager.setOrigin(currentOrigin.infos.icao, () => {
        ...
        fmc.onLeftInput[5] = () => {
            const value = parseInt(fmc.inOut);
